...
function editItem() {
	...
	if (row) {
		...
		$("#id").val(row.id);
		$("#name").val(row.name);
        $("#servicesList").val(row.servicesList);
        $("#price").val(row.price);
...
function createItem() {
	...
	$('#name').attr("value", "");
    $("#status").attr('value',"1")
    $("#servicesList").attr("value","");
    $("#price").attr("value","");
    $("#originalPrice").attr("value","");
    $("#servicesList").attr("value","");
...
function delItem() {
	var row = grid.datagrid('getSelected');
	if (row) {
		$.messager.confirm('警告', '您确认要删除：“' + row.name + '”吗?', function(r) {
			if (r) {
